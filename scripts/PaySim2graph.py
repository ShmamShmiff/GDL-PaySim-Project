...
def read_PaySim_and_compute_graph(accounts_dir, transactions_dir):
        ...
        for v in graph.predecessors(u):#loop though in degree nodes v
            ...
            if e_v2u:
                for edge in e_v2u: #for each edge from v to u
                    ...
                    src, dest, attrs = edge
                    edge_tsc_amts.append(attrs["amount"])
                    if attrs["isFraud"] == True:
                        is_sar = True
                ...
                if v2u_is_sar == True:
                    graph2.nodes[u]["isFraud"] = True
                    graph2.nodes[v]["isFraud"] = True
